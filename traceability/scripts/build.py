...
def write_open_problems(claims: dict):
    """Generate the open-problems registry view from role==open claims.

    The OPN-* claims are the canonical source of truth; this markdown is a derived
    view (do not hand-edit). 2-theory/open-problems.md points here.
    """
    ...
    lines = ["# Open Problems (generated)", "",
             ...
             "GitHub **Issues** track the *active subset* (those currently being worked) — filter "
             "[`label:open-problem`](https://github.com/jdlongmire/triadic-reality-theory/issues?q=is%3Aissue+is%3Aopen+label%3Aopen-problem). "
    ...
    for cid in opn:
        ...
        lines.append("")
        lines.append(cell(c.get("statement") or ""))
        if c.get("risk_if_false"):
            lines.append("")
            lines.append(f"*Risk if false:* {cell(c['risk_if_false'])}")
        lines.append("")
    lines += ["---", "",
              "**Testability** (paper §12 challenge 4) is tracked as the prediction claims "
              "`PRD-001` / `PRD-002`, not as an OPN problem (it is the discriminating output, not a belt task)."]
...
def main() -> int:
    ...
    print(f"loaded {len(claims)} claims ({n_open} open problems); generated 5 reports in {GEN_DIR.relative_to(ROOT)}/")
