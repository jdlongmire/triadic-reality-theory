...
**v3.2** executes Targets 3 and 5 (FLRW level) — results in the companion report [adce-t3t5-report.md](adce-t3t5-report.md), revised in-cycle after a second red-team pass ([reviews/2026-07-redteam-adce-t3t5/](../../reviews/2026-07-redteam-adce-t3t5/review.md)): **Target 3 closes negatively for the declared two-level structure** (A-SAT dissolves — identity-and-inert at the substrate level, false at the record level; a Jacobson-type horizon-thermodynamic level is *not* covered by the closure and merges with the extremal escape as one open problem); deliverable (a) is achieved **at background-morphology level only** by the feedback attractor ($H^* = \lambda/3$ recovered as an attractor of the diluted-stock bridge, not an assumption; costs: hand-set $\lambda \approx 3H_0$, DESI-disfavored quadrant, and a permanent ~14% dust-like early tracker with unestablished viability); the severe test **fails across the natural bridge-law grid — every cell**; a single probe (the scan and its inverse, one computation both ways) selects horizon coupling as the least-dead branch, and the CSG microdynamics motivates the feedback branch only under a named, open **inheritance conjecture**. The two branches are mutually exclusive in $w_a$ sign and DESI DR3/Euclid adjudicates between them. Criteria adjudicated this cycle: 4 fired-and-absorbed, 15 partially fired, 5 on watch, 12 acute on the horizon branch (report §7).
...
> **v3.2 closure (scoped).** Target 3 was executed and the axiom **dissolved**: at the substrate level ($N_P$) saturation is an identity of the number–volume correspondence — geometry-neutral, unable to force expansion (the static-universe result restated structurally); at the record level ($N_R$) saturation is false by ~100 orders of magnitude. Within the declared two-level structure, no level exists where the bound both binds and is geometry-sensitive, so the capacity-forcing route is closed *for that structure*, and A-SAT is retired. The closure is **not** universal (second red-team pass, vector 1): a mesoscopic Jacobson-type horizon-thermodynamic level (entropy saturated on causal horizons *and* geometry-sensitive) is not covered, and it coincides with the surviving horizon branch — that route and the extremal-principle exit have merged into one open problem. The extremal exit itself is weaker than this section hoped: Krishna–Mathew prove *law ⇒ entropy maximization*, not the converse. Full argument: [adce-t3t5-report.md](adce-t3t5-report.md) §2. All dynamical load now sits on the bridge law (§9.4).
...
5. **Target 4 (re-posed in v3.2): derive the bridge law.** With A-SAT retired, the growth-to-geometry theorem becomes: derive $B$ in $T^{(A)}_{\mu\nu} = B[N_R\text{-sector}; g_{\mu\nu}]$ from the actualization ontology — specifically a horizon-coupled form (the data-viable branch), for which the Jacobson-type route of the §9.1 closure note is the natural template. This is where the theory must do real work beyond the dictionary identity of §6.
...
4. Metric expansion follows only after assuming volume-count proportionality (equivalently: A-SAT can neither be derived nor independently motivated, §9.1). **[FIRED in v3.2 and absorbed:** the Target 3 execution established exactly this (A-SAT dissolved, [adce-t3t5-report.md](adce-t3t5-report.md) §2); the required substantial revision is the v3.1 reformulation plus the v3.2 bridge-law re-posing of Target 4 — the hypothesis no longer routes through A-SAT.**]**
...
v3.2 executed Targets 3 and 5 (FLRW level) — [adce-t3t5-report.md](adce-t3t5-report.md), revised in-cycle after a second red-team pass. Established: A-SAT dissolved and Target 3 closed negative for the declared two-level structure (§9.1 note; Jacobson-type horizon-thermodynamic route not covered, merged with the extremal escape); deliverable (a) achieved at background-morphology level (feedback attractor, $H^* = \lambda/3$ as fixed point; hand-set rate, disfavored quadrant, unestablished ~14% early tracker); severe test failed across the natural bridge-law grid, every cell; one probe (scan + its inverse) selects horizon coupling as the least-dead branch; CSG microdynamics motivates the feedback branch only under the open inheritance conjecture. Criteria adjudicated: 4 fired-and-absorbed, 15 partially fired, 5 on watch, 12 acute. The program now has exactly two damaged-but-live realizations and an externally scheduled kill condition (DESI DR3/Euclid, $w_a$ sign) for at least one of them.
